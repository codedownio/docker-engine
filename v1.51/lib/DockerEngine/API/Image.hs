...
-- | /Optional Param/ "keep-storage" - Amount of disk space in bytes to keep for cache  > **Deprecated**: This parameter is deprecated and has been renamed to \"reserved-space\". > It is kept for backward compatibility and will be removed in API v1.52. 
...
instance HasOptionalParam ImageGet Platform2 where
  applyOptionalParam req (Platform2 xs) =
    req `addQuery` toQuery ("platform", Just xs)
-- | @application/x-tar@
...
-- | /Optional Param/ "platform" - JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be saved if the image is multi-platform. If not provided, the full multi-platform image will be saved.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}` 
instance HasOptionalParam ImageGetAll Platform2 where
  applyOptionalParam req (Platform2 xs) =
    req `addQuery` toQuery ("platform", Just xs)
-- | @application/x-tar@
...
-- | /Optional Param/ "platform" - JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be load if the image is multi-platform. If not provided, the full multi-platform image will be loaded.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}` 
instance HasOptionalParam ImageLoad Platform2 where
  applyOptionalParam req (Platform2 xs) =
    req `addQuery` toQuery ("platform", Just xs)
