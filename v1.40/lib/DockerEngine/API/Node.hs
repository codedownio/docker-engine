...
nodeDelete
  ...
  -> DockerEngineRequest NodeDelete MimeNoContent NoContent MimeNoContent
nodeDelete (Id id) =
...
instance Produces NodeDelete MimeNoContent
...
-- | /Optional Param/ "filters" - Filters to process on the nodes list, encoded as JSON (a `map[string][]string`).  Available filters: - `id=<node id>` - `label=<engine label>` - `membership=`(`accepted`|`pending`)` - `name=<node name>` - `node.label=<node label>` - `role=`(`manager`|`worker`)` 
...
nodeUpdate
  ...
  -> VersionInteger -- ^ "version" -  The version number of the node object being updated. This is required to avoid conflicting writes. 
  -> DockerEngineRequest NodeUpdate contentType NoContent MimeNoContent
nodeUpdate _ (Id id) (VersionInteger version) =
...
instance Produces NodeUpdate MimeNoContent
