...
module DockerEngine.API.Task where
...
taskInspect
  ...
  -> DockerEngineRequest TaskInspect MimeNoContent Task MimeJSON
taskInspect (Id id) =
...
-- 
taskList
  :: DockerEngineRequest TaskList MimeNoContent [Task] MimeJSON
taskList =
  _mkRequest "GET" ["/tasks"]

data TaskList  

-- | /Optional Param/ "filters" - A JSON encoded value of the filters (a `map[string][]string`) to process on the tasks list.  Available filters:  - `desired-state=(running | shutdown | accepted)` - `id=<task id>` - `label=key` or `label=\"key=value\"` - `name=<task name>` - `node=<node id or name>` - `service=<service name>` 
...
taskLogs
  ...
  -> Id -- ^ "id" -  ID of the task
  -> DockerEngineRequest TaskLogs MimeNoContent FilePath accept
taskLogs  _ (Id id) =
...
instance Produces TaskLogs MimeVndDockerMultiplexedStream
