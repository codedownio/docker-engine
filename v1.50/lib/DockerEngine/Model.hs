{-
   ...
   Docker Engine API API version: 1.50
...
data ContainerUpdateRequest = ContainerUpdateRequest
  ...
  , containerUpdateRequestKernelMemoryTcp :: !(Maybe Integer) -- ^ "KernelMemoryTCP" - Hard limit for kernel TCP buffer memory (in bytes). Depending on the OCI runtime in use, this option may be ignored. It is no longer supported by the default (runc) runtime.  This field is omitted when empty. 
...
data HostConfig = HostConfig
  ...
  , hostConfigKernelMemoryTcp :: !(Maybe Integer) -- ^ "KernelMemoryTCP" - Hard limit for kernel TCP buffer memory (in bytes). Depending on the OCI runtime in use, this option may be ignored. It is no longer supported by the default (runc) runtime.  This field is omitted when empty. 
...
mkImageSummary
  ...
  -> Int -- ^ 'imageSummaryContainers': Number of containers using this image. Includes both stopped and running containers.  This size is not calculated by default, and depends on which API endpoint is used. `-1` indicates that the value has not been set / calculated. 
  -> ImageSummary
...
data NetworkSettings = NetworkSettings
  { networkSettingsBridge :: !(Maybe Text) -- ^ "Bridge" - Name of the default bridge interface when dockerd&#39;s --bridge flag is set. 
...
data Resources = Resources
  ...
  , resourcesKernelMemoryTcp :: !(Maybe Integer) -- ^ "KernelMemoryTCP" - Hard limit for kernel TCP buffer memory (in bytes). Depending on the OCI runtime in use, this option may be ignored. It is no longer supported by the default (runc) runtime.  This field is omitted when empty. 
